...
volatile uint8_t ManualDet = 0;      // set by S2, cleared by Task2 timer
...
#define SOUND_THRESHOLD 518  // Adjused this based on testing with microphone
...
// Outputs: none
void Task0(void){
    static uint32_t camHold = 0;    // in ms
    static uint32_t chirpTimer = 0; // for limiting chirps
    ...
    while(1) {
        ...
        if(c != -1) {  // If data received
            ...
            if (c == '1') {
                // Set detection flag and hold timer
                Saw = 1;
                camHold = CAM_HOLD_MS;
                OS_Signal(&GUIwake);  // Wake GUI immediately
                
                // Play detection chirp if not too frequent
                if (chirpTimer == 0) {
                    BSP_Buzzer_Set(512);  
                    BSP_Delay1ms(80);
                    BSP_Buzzer_Set(0);
                    chirpTimer = MIC_CHIRP_PERIOD_MS;  // Rate limit chirps
                }
            }
            else if (c == '0') {
...
void Task1(void){
    ...
    static uint32_t micHold = 0;    // hold detection display (in ms)
    static uint32_t nextChirpDue = 0;  // timestamp when next chirp allowed
    ...
    while(1){
        ...
        uint32_t now = Time;  // Current OS time in ms
        ...
        if(PlotState == Microphone) {
            ...
            if(SoundData > SOUND_THRESHOLD) {
                if(++aboveCnt == MIC_DEBOUNCE_SAMPLES) {
                    // First time we consider it "detected"
                    Heard = 1;
                    micHold = MIC_HOLD_MS;
                    OS_Signal(&GUIwake);  // Wake GUI immediately
                    aboveCnt = 0;   // Reset counter
                    
                    // Rate-limit the buzzer chirps
                    if(now >= nextChirpDue) {
                        BSP_Buzzer_Set(512);
                        BSP_Delay1ms(80);
                        BSP_Buzzer_Set(0);
                        nextChirpDue = now + MIC_CHIRP_PERIOD_MS;
                    }
                }
            } else {
            ...
            // Hold detection banner for a while after sound stops
            if(Heard && micHold) {
                if(--micHold == 0) {
                    Heard = 0;
                    OS_Signal(&GUIwake);  // Wake GUI immediately
...
void Task2(void){
    ...
    while(1){
        ...
        // No detection
        if(!Saw && !Heard && !ManualDet){
            BSP_LCD_PlotPoint(30, NOCOLOR);
            BSP_LCD_FillRect(25, 40, 90, 40, LCD_BLACK);
        } 
        // Camera detection - Camera has priority
        else if(Saw || (PlotState == Cam && ManualDet)){
        ...
        else if(Heard || (PlotState == Microphone && ManualDet)){
        ...
        // We still need to receive from mailbox occasionally to prevent buffer overflow
...
// Inputs:  none
// Outputs: none
// Modified Task3 to handle S2 button (But1) as a "press and hold" button
void Task3(void){
    ...
    while(1){
        ...
        // S2 button (But1) - manual detection while pressed
        But1 = BSP_Button2_Input();
        
        if(But1 == 0) {  // Button is currently pressed
            if(prev1 != 0) {  // Just pressed (edge detection)
                BSP_Buzzer_Set(512);  // Chirp on press
                BSP_Delay1ms(50);
                BSP_Buzzer_Set(0);
            }
            ManualDet = 1;  // Show detection while button is pressed
            OS_Signal(&GUIwake);  // Wake GUI to show detection
        } else {  // Button is released
            if(prev1 == 0) {  // Just released (edge detection)
                ManualDet = 0;  // Clear detection when button is released
                OS_Signal(&GUIwake);  // Wake GUI to update
            }
        }
        prev1 = But1;
        
        // S1 button - toggle mode
        ...
        if((But2 == 0) && (prev2 != 0)){
            ...
            if(PlotState == Cam){
                ...
                // Reset camera detection flag when switching to mic mode
                Saw = 0;
                ...
                // Reset microphone detection flag when switching to camera mode
                Heard = 0;
            }
            OS_Signal(&GUIwake);  // Wake GUI to update after mode change
            BSP_Buzzer_Set(512);
        }
        prev2 = But2;
